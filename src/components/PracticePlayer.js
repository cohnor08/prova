        ...
        {phase === 'play' && item ? (
          <>
            ...
            <ScrollView contentContainerStyle={styles.body} showsVerticalScrollIndicator={false}>
              ...
              {/* What the teacher attached when they set the task — sheet
                  music, a backing track to play along to (it plays right here,
                  and the clock keeps running), a clip. Keyed on the task so a
                  track stops when you move on, even if the next task uses the
                  same file. */}
              {item.kind === 'teacher' && (item.attachments || []).length > 0 && (
                <TaskAttachments key={item.id} attachments={item.attachments} style={styles.taskAtts} />
              )}
